...
def gen_query_code(server_name, users, guilds, starttime):
    ...
    if guilds:
        for guild in guilds:
            query += "g%s: reports(guildName: \"%s\",  guildServerSlug: \"%s\", guildServerRegion: \"eu\") { data { code }} \n" % (idx, guild, server_name)
            idx += 1

    query += "}}"
    return query

def gen_query_user(server_name, usernames, userdata):
    idx = 1
    partition = 3
    partition_name = "P3"
    ...
    if usernames:
        for name in usernames:
            query += "c%s: character(name: \"%s\", serverRegion:\"eu\", serverSlug: \"%s\") { id name classID " % (idx, name, server_name)
            for zone in zones:
                for size in ["10", "25"]:
...
def write_usernames(usernames):
    with open("usernames.txt", 'w') as file_handler:
...
def query_usernames(report_code):
    ...
    for key, reports in entries.items():
        if reports["rankedCharacters"]:
            ...
            for report in reports["rankedCharacters"]:
                if report["name"] not in usernames:
                    usernames.append(report["name"])
...
def update_userdata(server_id, server_name, usernames):
    ...
    while True:
        ...
        stop = False
        if idx + step >= len_usernames:
            end = len_usernames
            stop = True

        print("usernames(%s) = %s" % (len(usernames), usernames[idx:end]))
        print("\n\nget user data from %s to %s/%s" % (idx, end - 1, len_usernames))
...
def ant_run(server_id, server_name, userlist, guildlist, starttime):
    ...
    usernames = query_usernames(report_code)
    write_usernames(usernames)

    userdata = update_userdata(server_id, server_name, usernames)
